...
namespace CustomRandom
{
    public static class RandomChoice
    {
        ...
        public static T Choice<T>(IList<T> sequence, double[] probabilities)
        {
            return Choice(sequence, 1, probabilities).First();
        }
        public static int Choice(int upTo, double[] probabilities)
        {
            return Choice(Enumerable.Range(0, upTo).ToArray(), probabilities);
        }
    }
}